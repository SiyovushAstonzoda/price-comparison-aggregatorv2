...
import { fetchProductOffers } from '../api';
import { formatPrice } from './ProductCard';

/**
 * ProductDetailModal
 *
 * Fetches per-store offers for a given masterProductId and renders
 * them in a ranked list. The cheapest offer is highlighted.
 *
 * Note: /api/products/{id} returns PascalCase keys (untyped Dapper),
 * so we read Source, Price, ImageUrl, ProductUrl with PascalCase.
 */
export default function ProductDetailModal({ masterId, title, onClose }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative bg-white rounded-3xl p-7 w-full max-w-[560px] max-h-[85vh] overflow-y-auto shadow-[0_20px_30px_-10px_rgba(15,23,42,0.1),0_10px_15px_-5px_rgba(15,23,42,0.04)] border border-slate-200 animate-modal-scale">
        ...
        <p className="text-sm text-slate-500 mb-6 font-medium">Mağaza fiyatları karşılaştırması</p>
        ...
        <div id="offers" className="flex flex-col gap-3">
          ...
          {error && !loading && (
            <div className="py-10 text-center text-slate-500">Fiyatlar yüklenemedi.</div>
          )}

          {!loading && !error && offers.length === 0 && (
            <div className="py-10 text-center text-slate-500">Bu ürün için teklif bulunamadı.</div>
          )}
          ...
          {!loading && !error && offers.map((offer, index) => {
            ...
            const url      = offer.ProductUrl ?? offer.productUrl;
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-display font-bold text-[0.9375rem] text-slate-900 capitalize">
                    {source}
                  </div>
                  {index === 0 && (
                ...
                <span className="font-display font-extrabold text-lg text-emerald-600 whitespace-nowrap">
                  {formatPrice(price)}
                </span>
                {url && (
                  <a
